...
from utils.llm_client import groq_json_extract, get_llm_client

# ── Embedding model (now uses Ollama) ──────────────────────────────────

def get_embed_model():
    """Wrapper for LLMClient to maintain API compatibility."""
    ...
    def parse_contract(
        ...
        try:
            ...
            try:
                ...
                if should_close:
                    db.close()
            print("[ParserAgent] Embedding complete.")
        except Exception as e:
            import traceback
            print(f"[ParserAgent] ERROR in Stage 3: {e}")
            traceback.print_exc()
            raise e
        ...
        audit_path = f"/app/data/audit/extraction_audit_{contract_id}.json"
        with open(audit_path, "w", encoding="utf-8") as f:
            json.dump(audit_log, f, indent=2)

        if unresolved:
            unresolved_path = f"/app/data/audit/unresolved_fields_{contract_id}.json"
